...
source_control_table_path = source_bucket_path + '/control_table/'
target_control_table_path = target_bucket_path + '/control_table/'

delta_table_path = target_bucket_path + '/delta/'
...
for date in exec_dates:
    ...
    for obj in minio_client.list_objects(source_bucket_name, prefix=api_name + '/' + date + "/"):
    ...
    if df_date.count() != df_date.select("game_id").distinct().count():    
        
        print("Removendo registros duplicados...")

        window_spec = Window.partitionBy("game_id").orderBy(fn.col("extracted_datetime").desc())
        df_date = (
            df_date
    ...
    # Caso contrário, combina o DataFrame atual com o DataFrame anterior
        ...
        # Antes de realizar o merge, identifica quais são os registros e novos e atualizados (sem considerar a coluna extracted_datetime)
        df_delta = df_actual.toDF().drop("extracted_datetime").exceptAll(df_date.drop("extracted_datetime"))
        df_delta = df_delta.withColumn("extracted_datetime", fn.lit(df_date.select('extracted_datetime').first()[0]))
        
        # Realiza o merge entre o DataFrame atual e a tabela Delta            
        df_actual.alias("actualData") \
            .merge(source = df_delta.alias("updatedData"),
